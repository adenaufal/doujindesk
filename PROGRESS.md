...
|---|---|---|---|
| 1 | No mock data | `grep -rn "Mock\|mockUser\|mock-session" src/` | One hit, in a test file: `vi.clearAllMocks()` at `src/components/attendee.test.tsx:70` — Vitest's own API. Nothing in shipped source. Asserted permanently in `src/test/criteria.test.ts`, which also fails on `Comic Frontier 18`, `Sakura Studios`, `2500000000`, `lorem ipsum` and `trae-api-sg`, and boots `src/main.tsx` against a seeded localStorage to prove the five legacy `persist` keys are removed |
